...
for r in 1:size(Gs, 1), c in 1:size(Gs, 2)
    ...
    open("distances.bin", "w") do dfile
        ...
        @info "scoring links"
        scores[r, c] = score_links(x -> x < 1000, candidate_links[r, c], matrix, sub_origin_weights, weights, 1000)
    end
...
## Create the RUS link example

```{julia}
# note that this won't work if the OSM input data are changed
rus = collect(filter(l -> l.fr_edge_src == 134841 && l.to_edge_src == 173384, links))
@assert length(rus) == 1
@assert first(rus).geographic_length_m == 21

create_pbfs(
    joinpath(DATA_PATH, "data", "triangle.osm.pbf"),
    nothing,
    joinpath(DATA_PATH, "data", "triangle-rus.osm.pbf"),
    rus,
    G
)
```

## Create the figure that shows network partitioning

```{julia}
extent = MissingLinks.extent(G)
rowct = size(Gs, 1)
colct = size(Gs, 2)
row_size = (extent.max_y - extent.min_y) / rowct
col_size = (extent.max_x - extent.min_x) / colct
buffer = 1000

# Ref prevents Iterators.flatten from continuing into the tuple
geoms = DataFrame(:geometry => map(Iterators.flatten(Ref.(Iterators.product(1:colct, 1:rowct)))) do (c, r)
    xmin = extent.min_x + col_size * (c - 1)
    xmax = xmin + col_size
    ymin = extent.min_y + row_size * (r - 1)
    ymax = ymin + row_size
...
end, :id => 1:(rowct * colct))

GeoDataFrames.write(joinpath(DATA_PATH, "data", "partition_cells.gpkg"), geoms, crs=GFT.EPSG(32119))
```
