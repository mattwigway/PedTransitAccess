...
import MissingLinks: CandidateLink, TraversalRule
import OpenStreetMapPBF: write_pbf
import StatsBase: sample, cor, corspearman
...
```{julia}
traversal_settings = # In MissingLinks 0.2.3, this has highway=road included, which is arguably a mistake
    # this will be fixed in 0.2.4, but it's not actually a problem for this analysis
    # as there are no such ways in the input data.
    MissingLinks.RuleBasedTraversalPermissionSettings([
        # this is not in the MissingLinks default labeler, but is important for parity with R5
        TraversalRule("access" .=> ["no", "private", "destination" ,"customers" ,
            "delivery", "forestry" ,"agricultural",
                "residents" ,"resident",
                "customer", "private"], false),
        ...
        TraversalRule(["foot"] .=> "no", false),

        # things we assume are pedestrian
        TraversalRule([
                ("highway" .=> ["footway", "cycleway", "pedestrian", "track", "sidewalk", "service", "living_street", "steps", "path", "crossing", "residential"])...,
...
```{julia}
Gs = MissingLinks.partition(G, 6, 4, 1450)
...
for r in 1:size(Gs, 1), c in 1:size(Gs, 2)
    ...
    open("distances.bin", "w") do dfile
        ...
        score_matrix_gravity[r, c] = score_links(x -> x > 1350 ? 0 : exp(x * -0.00344635), Gsub, candidate_links[r, c], matrix, sub_origin_weights, weights, 1350)
    end
...
```{julia}
cor(scores, scores_gravity)
```

```{julia}
corspearman(scores, scores_gravity)
```
...
GeoDataFrames.write(
    ...
    links_to_gis(G, links, "score"=>scores, "score_gravity" => scores_gravity,  "rank" => ranks, "id" => ids)
)
```

## Insert top 175 links into network (empirically determined)
...
## Create individual PBFs for a random sample of links
...
for (idx, linkid, link, rank) in zip(1:total, ids[link_indices], links[link_indices], ranks[link_indices])
    ...
    dir = joinpath(DATA_PATH, "networks", "links", "link_id_$linkid")
    if !isdir(dir)
        mkpath(dir)
    end
...
## Create individual PBFs for the top 250 links

```{julia}
link_indices = eachindex(links)[ranks .≤ 250]
# NB LibSpatialIndex is used here and is not threadsafe, do not parallelize this loop
for (idx, linkid, link, rank) in zip(1:length(link_indices), ids[link_indices], links[link_indices], ranks[link_indices])
    # shallow copies, for use with copy-on-write operations
    ways = copy(base.ways)
    nodes = copy(base.nodes)
    insert_links!(G, ways, nodes, [link], idx = sidx)
    dir = joinpath(DATA_PATH, "networks", "links_top250", "link_id_$linkid")
    if !isdir(dir)
        mkpath(dir)
    end
    ...
    if idx % 10 == 0
        @info "Wrote $idx / $sample_size top150 OSM files"
    end
end
```
