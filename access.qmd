...
```{r}
routes = map(Sys.glob(file.path(DATA_PATH, "data", "gtfs", "*.zip")), function (fn) {
    gtfs = read_gtfs(fn)
    convert_shapes_to_sf(gtfs, shape_id = unique(gtfs$trips$shape_id), sort_sequence=TRUE)
}) |>
    list_rbind()
```
...
walk_iso_map = tm_basemap(server="Esri.WorldImagery", alpha=0.5) +
    ...
    tm_shape(mutate(stops, label="Other bus stops")) +
        tm_dots(fill="label", size=0.5, fill.scale = tm_scale(values=c("darkgreen")), fill.legend=tm_legend(show=T, title="", position=tm_pos_out("right", "center"))) +
    tm_shape(mutate(stop_loc, label="Stop")) +
        tm_dots(fill="label", size=0.75, fill.scale = tm_scale(values=c("blue")), fill.legend=tm_legend(show=T, title="", position=tm_pos_out("right", "center"))
            ) +
    tm_shape(mutate(ccc_link, label="New link")) +
        tm_lines(lwd=3, col="label", col.scale = tm_scale(values=c("red")), col.legend=tm_legend(show=T, title="", position=tm_pos_out("right", "center"))
            )
...
walk_iso_map_wide = tm_basemap(server="Esri.WorldImagery", alpha=0.5) +
    ...
    tm_shape(mutate(ccc_link, label="New link")) +
        tm_lines(lwd=6, col="label", col.scale = tm_scale(values=c("red")), col.legend=tm_legend(show=T, title="", position=tm_pos_out("right", "center"))
            ) +
    tm_shape(mutate(stops, label="Other bus stops")) +
        tm_dots(fill="label", size=0.5, fill.scale = tm_scale(values=c("lightblue")), fill.legend=tm_legend(show=T, title="", position=tm_pos_out("right", "center"))) +
    tm_shape(mutate(stop_loc, label="Stop")) +
        tm_dots(fill="label", size=0.75, fill.scale = tm_scale(values=c("blue")), fill.legend=tm_legend(show=T, title="", position=tm_pos_out("right", "center"))
            ) +
...
acc_map = tm_basemap(
  server = paste0(
    "https://api.mapbox.com/styles/v1/mapbox/light-v11/tiles/512/{z}/{x}/{y}@2x?fmt=.png&access_token=",
  ...
  tm_polygons(
    ...
    fill.scale = tm_scale_intervals(
      style = "fixed",
      breaks = c(1, 500, 1000, 5000, 10000, 220000),
      # drop the lightest colors for legibility
      values = rev(viridis::mako(8)[1:6])
    ),
    fill_alpha = 0.75,
    col_alpha = 0,
    tm_legend(
      title = "Increase in Job Access",
      position = tm_pos_in("right", "top")
    )
  ) +
  tm_shape(routes |> mutate(label="Bus routes")) +
  tm_lines(col="label", col.scale = tm_scale(values = c("#777777")), col.legend = tm_legend(title = "", position = tm_pos_in("right", "top"))) +
  tm_shape(filter(links, rank <= 100) |> mutate(label = "New links")) +
  tm_dots(
    fill = "label",
    fill.scale = tm_scale(values = c("red")),
    fill.legend = tm_legend(title = "", position = tm_pos_in("right", "top"))
  ) +
  tm_scalebar() +
  tm_credits("Basemap © OpenStreetMap contributors, Mapbox") +
  tm_logo(
    here::here("paper/figures/mapbox-logo-black.png"),
    height = 1.5,
...
tmap_save(
  acc_map,
  here::here("paper", "figures", "regional.png"),
  width = 12,
  height = 8,
  bg = "white"
)
