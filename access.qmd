...
```{r}
single_link_map = tmap_arrange(walk_iso_map + tm_credits("Basemaps © OpenStreetMap contributors, Esri, Mapbox", position=tm_pos_out("center", "bottom")), tr_iso_map)
single_link_map
...
block_acc = blocks |>
    ...
    mutate(acc_inc = acc_scenario - acc_base)
```
...
tm_basemap(server=paste0("https://api.mapbox.com/styles/v1/mapbox/streets-v12/tiles/512/{z}/{x}/{y}@2x?fmt=.png&access_token=", Sys.getenv("MAPBOX_API_KEY"))) +
    tm_shape(isos) + # just to get the scale right
    tm_shape(filter(block_acc, acc_inc > 0)) +
    tm_polygons(fill="acc_inc", fill_alpha=0.75, col_alpha=0) +
    tm_scalebar()
```
...
```{r}
problem_pts = st_transform(st_centroid(block_acc[block_acc$acc_inc < -1e-6,]), 4326)

problem_base_iso = isochrone(
    base,
    base,
    problem_pts,
    mode = "WALK",
...
```{r}
rbind(problem_base_iso, problem_scen_iso) |>
    write_sf(file.path(DATA_PATH, "data", "problem_isos.gpkg"))
```

```{r}
v = street_network_to_sf(base)$vertices
v$id = v$index

problem_tt_base = travel_time_matrix(
    base,
    base,
    problem_pts,
    v,
...
problem_tt_scen = travel_time_matrix(
    scenario,
    scenario,
    problem_pts,
    v,
...
problem_tt = problem_tt_base[problem_tt_scen[,.(from_id, to_id, travel_time_p50_scen=travel_time_p50)], on=c("from_id", "to_id")]

problem_tt_sf = problem_tt |>
    left_join(mutate(v, id=as.character(id)), by=c("to_id"="id"))

problem_tt_sf |>
    write_sf(file.path(DATA_PATH, "data", "travel_time_problems.gpkg"))
```
